RSpec.shared_examples 'permissions manager' do |factory, options|
  let(:model) { create(factory, options) }
  ...
  describe '#add_with_objects' do
    ...
    it 'adds the id of the object to the permissions' do
      ...
      expect(model.permission.reload.inventories_items_read_only).to include(object.id)
    end
  end
  ...
  describe '#remove_with_objects' do
    ...
    before do
      model.permission.update(permission_name => [object.id])
    end

    it 'removes the id of the object from the permission' do
      subject.remove_with_objects(permission_name, [object.id])
      expect(model.permission.reload.inventories_items_read_only).to_not include(object.id)
    end
  end
  ...
  describe '#add' do
    ...
    before do
      model.permission.update(permission_name => false)
    end

    it 'sets the permission as true' do
      subject.add(permission_name)
      expect(model.permission.reload.send(permission_name)).to be_truthy
    end
  end
  ...
  describe '#remove' do
    ...
    before do
      model.permission.update(permission_name => true)
    end

    it 'sets the permission as true' do
      subject.remove(permission_name)
      expect(model.permission.reload.send(permission_name)).to be_falsy
    end
  end
