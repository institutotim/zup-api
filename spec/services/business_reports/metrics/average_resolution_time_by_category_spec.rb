...
describe BusinessReports::Metrics::AverageResolutionTimeByCategory do
  let(:date_range) do
  ...
  describe '#fetch_data' do
    ...
    before do
      final_status1 = category1.status_categories.final.first.status
      final_status2 = category2.status_categories.final.first.status

      reports1.each do |report|
        Reports::UpdateItemStatus.new(report).update_status!(final_status1)
        report.update(resolved_at: report.created_at + 20.days)
      end

      reports2.each do |report|
        Reports::UpdateItemStatus.new(report).update_status!(final_status2)
        report.update(resolved_at: report.created_at + 13.days)
      end
    end
